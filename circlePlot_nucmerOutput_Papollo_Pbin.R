...
setwd("/mnt/griffin/chrwhe/SEPA/apollo_alignment")
# Prefix
Prefix <- "nucmer_aln_Pbin_v_Papo"

Ref_pal <- read_table("Pbin_contigs.txt", col_names = F) %>%
   rename(Chr = X1) %>%
  filter(Chr != "Mitochondria") %>%
  mutate(color = rep(brewer.pal(n=11, "Spectral"), length.out = length(Chr)))

Qry_pal <-read_table2("Papo_contigs.txt", col_names = F) %>%
  rename(Chr = X1) %>%
  mutate(color = "#FFFFFF")
...
Alignment <- read_csv("nucmer_aln_Pbin_v_Papo.qr1_filter.reformat.coords", col_names = nucmer_coord_colnames2 )
...
Alignment3 <- Alignment2 %>% filter(lenQ >= 10000000 & lenR >= 1000000 & L1 >= t_len & Percent <= t_per)
...
pdf(paste0(Prefix,"_circosPlot_",t_per,"id_", t_len, "L1_1MlenR_10MlenQ.pdf"), width = 10, height=7)
par(mar = c(1, 1, 1, 1))
